...
**Knowledge-base governance plan (required if the agent uses intelligence — RAG, an IGM-governed substrate, a domain graph, or any retrieval-grounded reasoning).** An agent that draws on a knowledge base to ground its reasoning inherits the governance state of that knowledge base. A Stage 1 brief that names "use a knowledge base" without specifying how that knowledge base will be governed has deferred a load-bearing decision into the implementation phase, where it becomes invisible to product-level review. The Stage 1 minimum requires a knowledge-base governance plan with four artefacts:

- **Identified IGM authorities.** The named individuals (or role assignments) who will hold the four IGM authorities — Semantic, Assertion, Inference, Revision — for each claim family the agent will consume. "To be assigned" is not a valid entry; if the authority cannot be named, the claim family cannot be relied on at Stage 1 minimum. See `intelligence-governance-manifesto/manifesto-principles.md` for the authority definitions.
- **Planned domain scope.** The claim families and subdomains the agent will consume, with the boundary of what is in scope and what is explicitly out of scope. An agent whose domain scope is "everything in the corporate knowledge base" has not specified scope — it has declined to.
- **Planned ingestion sources.** The sources from which claims will be ingested (regulatory publications, internal procedure documents, vendor data feeds, expert-validated entries, prior agent outputs). For each source, the planned acquisition mode (Harvest / Pull / Push / Author) and the planned validation regime before promotion to higher epistemic tiers.
- **Planned epistemic-tier policies.** The minimum epistemic tier required for the agent to act on a claim, per consequence class of the action. An agent that may execute Critical-consequence actions on Provisional-tier claims has no tier policy; the gate fails. An agent acting only at Low consequence on Confirmed+ claims has a coherent policy that can be reviewed.

This artefact is the Stage 1 link to IGM governance. Without it, Stage 4's behavioral baseline cannot be measured against substrate state, Stage 5's drift detection cannot distinguish substrate-caused from agent-caused deterioration, the knowledge-base component of the CSH (per `aplc.md` Section 1) cannot be computed against a defined claim set with defined governance metadata, and the Initiative Authorization Gate's substrate-depth condition has no domain to assess against. The plan does not need to be at IGM operational maturity at Stage 1 minimum — it needs to be present, with named authorities, scope, sources, and tier policy specific enough that Stage 2 can derive constraint requirements from it. For agents that do not use intelligence (a deterministic agent with no retrieval, no claim base, no substrate dependence), this artefact is replaced with a one-line declaration "the agent does not use intelligence" so that the absence is recorded rather than implied.

**Why this is the floor.** Everything in Stages 2 through 7 traces back to Stage 1. A behavioral specification without a product brief is a technical document without a product purpose. An evaluation portfolio without a user model is testing the wrong population. A conformity assessment without a regulatory classification is filling out forms for an unknown requirement. A drift-detection programme without a knowledge-base governance plan cannot tell substrate decay from agent regression. The Stage 1 minimum is not bureaucracy; it is the prerequisite for everything else being coherent.
